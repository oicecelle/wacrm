...
const ROLE_DESCRIPTIONS: Record<InviteRole, string> = {
  admin:
    'Pode convidar membros da equipe, gerenciar configurações, enviar mensagens e editar dados.',
  agent:
    'Pode usar a caixa de entrada, contatos, disparos, automações e fluxos. Sem acesso a configurações ou membros.',
  viewer: 'Acesso somente leitura em todas as telas. Não pode enviar nem editar nada.',
};
...
export function InviteMemberDialog({
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const message = `Você foi convidado(a) para ${accountName} no wacrm. Entre por este link (válido por ${result?.expiresInDays} dia${result?.expiresInDays === 1 ? '' : 's'}): ${url}`;
    ...
    >
      <DialogContent className="bg-popover border-border sm:max-w-md">
        {result ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Compartilhe esse link com o novo membro da equipe. Ele(a) vai
                poder criar conta (ou entrar) e ingressar na conta como{' '}
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="rounded-md border border-amber-500/50 bg-amber-500/15 px-3 py-2 text-xs text-amber-200">
                <strong className="font-semibold text-amber-100">
                  Salve esse link agora.
                </strong>{' '}
                Nunca guardamos o texto original — assim que você fechar esse
                diálogo, a URL some. Pra compartilhar de novo, revogue esse
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">Convidar um membro da equipe</DialogTitle>
              <DialogDescription className="text-muted-foreground">
                Gere um link de convite de uso único. Compartilhe pelo
                WhatsApp, Slack, ou qualquer canal que preferir — não
                depende de serviço de e-mail.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Ajuda a lembrar pra quem você mandou o link, na lista de
                  pendentes abaixo.
                </p>
              </div>
            </div>
