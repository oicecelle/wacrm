...
// routes. Meant to be called every 15-30 minutes by cron-job.org.
//
...
// tolerance window so a 15-30min polling interval doesn't miss it),
// and last_run_date isn't already today. Each matching contact
// (found live via contact_tags, not a saved list — a contact tagged
// five minutes before the campaign runs still gets included) gets
// the configured message, then the "after" tag bookkeeping so the
// same contact isn't re-sent tomorrow.
export async function GET(request: Request) {
  ...
  if (campaignsErr) {
    console.error('[cron/scheduled-campaigns] Error fetching campaigns:', campaignsErr.message)
  ...
  for (const campaign of campaigns ?? []) {
    ...
    try {
      ...
      if (!wsConfig) {
        console.error(`[cron/scheduled-campaigns] No WhatsApp config for account ${campaign.account_id}`)
        await admin
          .from('scheduled_campaigns')
      ...
      for (const contact of contacts ?? []) {
        ...
        try {
          ...
          console.error(`[cron/scheduled-campaigns] Failed for contact ${contact.id}:`, err)
          stats.failed++
      ...
      console.error(`[cron/scheduled-campaigns] Campaign ${campaign.id} failed:`, err)
      await admin
        .from('scheduled_campaigns')
